...
void bug_zapper(System& slurp){
	...
	for (int k = 0; k < zapperWidth; k++){
		...
		for (int l = 0; l < zapperHeight; l++){
			Particle zapperElectric(temp_x, temp_y, 0, 0, INT_MAX, 125, 249, 255, Particle::STREAMER); // Makes Solid Blue Electricity for Zapper
...
void bane_of_Saitama(System& slurp){
	int X = slurp.get_scrnWidth();
	int Y = slurp.get_scrnHeight();

	Particle Bzzzt(X / 10, Y / 6, 10000, 10000, 200, 255, 85, 0, Particle::MOSQUITO);
