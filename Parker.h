...
static void parker_gasParticle() {
	System Gas;
	...
	for (int i = 0; i < 120; i++) {
		Sys.addParticle(Particle( ( horizontalFifth + (rand() % (3 * horizontalFifth))  ), ( (2 * verticalThird) + (rand() % verticalThird)  ) , 0 , (((rand() % 5) + 1) / 5), 50 + (rand() % 15) - 7, 200 + (rand() % 56), 20, GAS));
		Sys.sysUpdate();	
	}
}
